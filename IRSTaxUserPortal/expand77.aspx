...
<title>IRStaxRecords.com Services and FAQ</title>
...
<style>
  ...
  .itr-foot h3 { color: #00498C; font-weight: 700; margin-bottom: 10px; }
  .itr-foot p { color: #333; margin-bottom: 25px; font-size: 1.1rem; }
  .itr-cta-row { display: flex; justify-content: center; gap: 20px; flex-wrap: wrap; }
  .itr-btn { display: inline-block; padding: 12px 28px; font-size: 1rem; font-weight: 600; border-radius: 6px; text-decoration: none; transition: all .3s ease; }
  .itr-btn-primary { background-color: #00498C; color: #fff; border: none; }
  .itr-btn-primary:hover { background-color: #5dade2; color: #fff; }
  .itr-btn-ghost { background-color: transparent; color: #00498C; border: 2px solid #00498C; }
  .itr-btn-ghost:hover { background-color: #5dade2; color: #fff; border-color: #5dade2; }
  /* NEW: navy solid button (used only for “Talk to an Expert”) */
  .itr-btn-navy { background:#0b2d6b; color:#fff; border:1px solid #0b2d6b; }
  .itr-btn-navy:hover { background:#ffffff; color:#fff; border-color:#08224f; color:#000000; }
</style>
...
<style>
  ...
  :root{
    --bg:#E7EBEF; --txt:#1a1a1a; --muted:#555; --row:#f9fbff; --row-hover:#f0f6ff; --edge:#dce6f5; --neon:#0077ff; --radius:12px;
  }
  body{
    margin:0; background:var(--bg); color:var(--txt); font:15px/1.5 system-ui, sans-serif;
    /* was padding:40px 16px; — removed top padding so Services sits closer to banner */
    padding:0 16px;
  }
  .wrap{width:min(1100px,95vw); margin:0 auto}
  .title{margin:0 0 16px; font-weight:700}
  ...
  thead th{ text-align:left; font-weight:600; color:var(--muted); padding:14px 18px; border-bottom:1px solid var(--edge); background:#f6f9ff; }
  tbody tr.data{ transition:background .25s }
  tbody tr.data:hover{ background:var(--row-hover) }
  ...
  .lm-btn{
    ...
    color:var(--neon); text-decoration:none; font-weight:500; transition:color .15s ease;
  }
  .lm-btn:hover{ color:#005ecc }
  .lm-btn:focus-visible{ outline:2px solid var(--neon); outline-offset:3px; border-radius:4px }
...
<style>
  ...
  .itr-services{
    --itr-bg:#ffffff; --itr-ink:#0b1320; --itr-muted:#6a7280;
    --itr-brand:#1f6fff; --itr-brand-ink:#0d47a1; --itr-ring:rgba(31,111,255,.25);
    --itr-line:#e7eaf0; --itr-card:#ffffff; --itr-card-shadow:0 8px 28px rgba(2,12,44,.06);
    background:var(--itr-bg); color:var(--itr-ink);
    padding:12px 16px 48px;   /* &#8595; from 48px: pulls Services closer to the banner */
  }
  .itr-wrap{ max-width:1160px; margin:0 auto; }

  /* Hero card for the section heading/intro */
  .itr-card-hero{
    background:var(--itr-card); border:1px solid var(--itr-line); border-radius:16px;
    box-shadow:var(--itr-card-shadow); padding:22px; margin-bottom:20px;
  }
  .itr-card-hero h2{ font-size:32px; line-height:1.2; margin:0 0 8px; letter-spacing:.2px; }
  .itr-card-hero .itr-sub{ font-size:16px; color:var(--itr-muted); margin:0 0 8px; }
  .itr-card-hero .itr-intro{ font-size:17px; line-height:1.6; margin:0; max-width:900px; color:var(--itr-ink); }

  .itr-grid{ display:grid; grid-template-columns:1fr; gap:18px; margin:24px 0 10px; }
  @media (min-width:900px){ .itr-grid{ grid-template-columns:1fr 1fr; gap:20px; } }

  .itr-card{
    background:var(--itr-card); border:1px solid var(--itr-line); border-radius:16px;
    padding:22px; box-shadow:var(--itr-card-shadow); position:relative;
  }
  .itr-card-tag{
    position:absolute; right:16px; top:16px; font-size:12px;
    background:rgba(31,111,255,.08); color:var(--itr-brand); border:1px solid rgba(31,111,255,.25);
    padding:4px 8px; border-radius:999px;
  }
  .itr-card h3{ margin:0 0 6px; font-size:22px; line-height:1.3; }
  .itr-card-lead{ margin:0 0 12px; color:var(--itr-muted); font-size:16px; }

  .itr-list{ list-style:none; padding:0; margin:0 0 18px; }
  .itr-list li{
    display:flex; align-items:flex-start; gap:10px; padding:8px 0; border-top:1px dashed var(--itr-line);
  }
  .itr-list li:first-child{ border-top:none; }
  .itr-bullet{ min-width:20px; line-height:1.2; }

  .itr-cta-row{ display:flex; flex-wrap:wrap; gap:10px; }
  .itr-btn{
    ...
    transition:transform .06s ease, background-color .15s ease, border-color .15s ease; will-change:transform;
  }
  .itr-btn:hover{ transform:translateY(-1px); }
  .itr-btn-primary{ background:var(--itr-brand); color:#fff; border-color:var(--itr-brand); }
  .itr-btn-primary:hover{ background:var(--itr-brand-ink); border-color:var(--itr-brand-ink); }
  .itr-btn-ghost{ background:#fff; color:var(--itr-ink); }
  .itr-btn:focus{ outline:none; box-shadow:0 0 0 4px var(--itr-ring); }
/* Navy outline for the four ghost buttons inside the two service cards */
  ...
  .itr-btn:focus{ outline:none; box-shadow:0 0 0 4px var(--itr-ring); }
  .itr-proof{
    display:grid; grid-template-columns:repeat(2,1fr); gap:12px; margin:20px 0 24px;
  }
  @media (min-width:780px){ .itr-proof{ grid-template-columns:repeat(4,1fr); } }
  .itr-proof-item{ text-align:center; background:#fafbff; border:1px solid var(--itr-line); border-radius:14px; padding:16px 10px; }
  .itr-proof-num{ font-size:20px; font-weight:700; margin-bottom:4px; }
  .itr-proof-label{ font-size:12.5px; color:var(--itr-muted); }

  .itr-services .itr-foot{ text-align:center; border-top:1px solid var(--itr-line); padding-top:18px; background:transparent; }
  .itr-services .itr-foot h3{ margin:0 0 6px; font-size:22px; color:inherit; }
  .itr-services .itr-foot p{ margin:0 0 14px; color:var(--itr-muted); font-size:16px; }
</style>
...
<body bgcolor="#E7EBEF">
    ...
    <div class="hero">
	  ...
	  <main class="site-main"><map name="FPMap1">
  <area href="expand7.asp#details" shape="rect" coords="375, 145, 648, 182">
  <area href="cc.asp" shape="rect" coords="677, 146, 869, 180">
  <area href="wizard7.asp" shape="rect" coords="901, 147, 1304, 183">
  <area href="login2.aspx" shape="rect" coords="1374, 143, 1517, 182"></map><img src="https://www.irstaxrecords.com/new/images/top2.jpg" alt="IRS Tax Records banner" width="1902" height="190" usemap="#FPMap1"
    ...
    <section class="itr-services">
      <div class="itr-wrap">
        ...
        <div class="itr-grid">
          ...
          <article class="itr-card">
            ...
            <div class="itr-cta-row">
              <a href="/wizard7.asp" class="itr-btn itr-btn-ghost">Order Transcripts</a>
          ...
          <article class="itr-card">
            ...
            <div class="itr-cta-row">
              <a href="/wizard7.asp" class="itr-btn itr-btn-ghost">Verify Identity</a>
              <a href="#details" class="itr-btn itr-btn-ghost">Service Details</a>
            </div>
          </article>
        </div>
        ...
        <footer class="itr-foot">
          ...
          <div class="itr-cta-row">
            <a href="/wizard7.asp" class="itr-btn itr-btn-primary">Create Your Secure Account</a>
<!-- Button (no href to navigate) -->
<button type="button"
        class="itr-btn itr-btn-navy"
        onclick="return openCenteredPopup('/talktoanexpert.asp','TalkToAnExpert',800,740);">
  Talk to an Expert
</button>
...
function openCenteredPopup(url, title, w, h){
  ...
  if (!win) { window.location.assign(url); return false; } // popup blocked → fallback
...
>
  ...
  <!-- Full-bleed footer image at the very bottom --><!-- Full-bleed footer image at the very bottom -->
  <div class="footer-banner">
  </main><map name="FPMap0">
        <area href="mailto:info@irstaxrecords.com" shape="rect" coords="398, 229, 590, 255">
        <area href="https://www.irstaxrecords.com" shape="rect" coords="391, 44, 690, 152">
        <area href="expand7.asp" shape="rect" coords="804, 119, 1130, 143">
        <area href="expand7.asp#details" shape="rect" coords="806, 148, 1112, 167">
        <area href="expand7.asp.asp" shape="rect" coords="804, 176, 1041, 193">
        <area href="/wizard7.asp" shape="rect" coords="804, 203, 1131, 230">
        <area href="/wizard7.asp" shape="rect" coords="1277, 109, 1466, 155">
        <area href="expand7.asp" shape="rect" coords="1274, 169, 1540, 187">
        <area href="forms.asp" shape="rect" coords="1270, 198, 1618, 214">
        <area href="forms.asp" shape="rect" coords="1266, 222, 1604, 242"></map><img src="https://www.irstaxrecords.com/new/images/Footer.jpg" alt="IRS Tax Records footer" width="1902" height="289" usemap="#FPMap0">
  </div>
